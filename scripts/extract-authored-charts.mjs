...
if (!dir) {
  console.error('usage: extract-authored-charts.mjs --dir <transcripts> [--since ISO] [--until ISO] [--out file]')
  process.exit(1)
}
...
for (const f of readdirSync(dir).filter(n => n.endsWith('.jsonl'))) {
  ...
  for (const line of readFileSync(path, 'utf8').split('\n')) {
    ...
    if (!prompt && ev.type === 'user') {
      ...
      if (typeof c === 'string') {
        ...
        prompt = c.filter(x => x.type === 'text').map(x => x.text).join('\n')
      }
    }
    if (ev.type === 'assistant' && Array.isArray(ev.message?.content)) { // ACCESSOR — adapt here if needed
      for (const c of ev.message.content) {
        if (c.type === 'tool_use' && /StructuredOutput/i.test(String(c.name)) && c.input && typeof c.input === 'object') {
          structured = c.input
        }
      }
    }
  ...
  if (structured && typeof structured.finalDsl === 'string') {
    ...
    if (id && dsl) {
      judges.push({ id, expectedType: expectedType ?? null, finalDsl: dsl })
    }
  }
}
...
for (const a of authors) {
  ...
  if (!j) {
    console.warn(`UNMAPPED author persona=${a.persona} type=${a.structured.chartTypeChosen}`)
    continue
  }
  charts.push({
    id: j.id,
...
if (charts.length < 45) {
  console.error(`only ${charts.length} charts recovered (<45) — aborting`)
  process.exit(1)
}
