...
// lib 0.1.30's annotation allowlist flags `dy` as unknown, but the renderer
// honors it as a vertical text offset and two canonical samples (bitcoin-price,
// unemployment-rates) ship it. Surfacing it would teach that valid samples are
// broken, so we drop just this one known-honored property. Everything else
// under `unknown-annotation-property` (e.g. a range's `fromX`) still surfaces.
